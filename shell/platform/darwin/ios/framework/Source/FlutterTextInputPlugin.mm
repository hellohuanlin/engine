...
- (void)takeSnapshot {
  ...
  //  UIView *snapshot = [[UIApplication sharedApplication].windows[1]
...
- (void)onPointerMoveInScrollView:(NSDictionary*)dict {
  NSLog(@"onPointerMoveInScrollView in engine, should take snapshot if needed, and move the fake "
        @"keyboard if needed");
  ...
  self.fakeKeyboardViewContainer.frame =
      CGRectMake(0, fakeKeyboardY, keyboardWidth, keyboardHeight);
}
  ...
  [UIView animateWithDuration:0.5
      ...
      completion:^(BOOL finished) {
        ...
        if (shouldShowRealKeyboard) {
          ...
          // So we have to manually trigger another delegate function to tell framework to unfocus
          // in UI.
          [self hideTextInput];
